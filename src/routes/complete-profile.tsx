...
export const Route = createFileRoute("/complete-profile")({
  head: () => ({
    meta: [
      { title: "Complete Your Profile | Grafted" },
      { name: "description", content: "Complete your Grafted member profile." },
...
function CompleteProfilePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Leave the form blank if local storage is not readable.
    }
  }, []);
  ...
  return (
    <>
      <PageHero eyebrow="Member Profile" title="Complete your profile." subtitle="This becomes the first version of your Grafted member profile. Public fields can support the future directory. Private metrics stay Grafted-only." />
      <Section tone="sand">
        <form onSubmit={submit} className="mx-auto max-w-5xl rounded-2xl border border-border bg-background p-6 shadow-sm md:p-8">
          <div className="grid gap-8 lg:grid-cols-2">
            <div className="space-y-5">
              ...
              <label><span className={labelClass}>Logo URL</span><input className={fieldClass} value={form.logo_url} onChange={(e) => update("logo_url", e.target.value)} /></label>
              <label><span className={labelClass}>Headshot URL</span><input className={fieldClass} value={form.headshot_url} onChange={(e) => update("headshot_url", e.target.value)} /></label>
              <label><span className={labelClass}>Headshot visibility</span><select className={fieldClass} value={form.headshot_visibility} onChange={(e) => update("headshot_visibility", e.target.value)}><option value="public">Public</option><option value="member-only">Member-only</option></select></label>
