...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      { title: "Member Profile | Grafted" },
      { name: "description", content: "View your Grafted member profile." },
  ...
  return (
    <>
      <PageHero
        ...
        title="Your Grafted profile."
        subtitle="This is the first version of the member profile. Billing, directory access, and richer member tools will connect as Stripe and the portal come online."
      />
        ...
        <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-border bg-background p-6 shadow-sm md:p-8">
            <div className="flex items-start gap-4">
              <div className="inline-flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-full bg-deep-waters text-refined-gold">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild className="bg-deep-waters text-river-sand hover:bg-still-pool font-eyebrow text-xs uppercase tracking-[0.2em]">
          ...
          <aside className="space-y-5">
            <div className="rounded-2xl border border-refined-gold/35 bg-river-pale p-6">
              <div className="flex items-center gap-3 font-serif text-xl text-deep-waters">
                <CreditCard className="h-5 w-5 text-refined-gold" /> Billing status
              </div>
              ...
              <div className="mt-4 rounded-lg bg-background px-4 py-3 text-sm text-deep-waters/75">
                Current status: <strong>{profile?.payment_status || "stripe_placeholder"}</strong>
              </div>
            </div>
            <div className="rounded-2xl border border-border bg-background p-6">
              <div className="flex items-center gap-3 font-serif text-xl text-deep-waters">
                <BadgeCheck className="h-5 w-5 text-refined-gold" /> Founding member
              </div>
              <p className="mt-3 text-sm leading-relaxed text-deep-waters/75">
                The first 15 paid signups will receive founding member status once Stripe payment timestamps are connected.
              </p>
            </div>
          </aside>
        </div>
...
function ProfileItem({ label, value }: { label: string; value?: string | number | null }) {
  return (
    <div className="rounded-xl border border-deep-waters/10 bg-river-pale p-4">
      <div className="font-eyebrow text-[10px] uppercase tracking-[0.22em] text-deep-waters/50">{label}</div>
      <div className="mt-2 text-sm text-deep-waters/80">{value || "Not added yet"}</div>
    </div>
  );
}
...
function ProfileBlock({ label, value }: { label: string; value?: string | null }) {
  return (
    <div>
      <div className="font-eyebrow text-[10px] uppercase tracking-[0.22em] text-refined-gold">{label}</div>
      <p className="mt-2 text-deep-waters/80">{value || "Not added yet"}</p>
    </div>
  );
}
