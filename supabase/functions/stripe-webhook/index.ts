...
async function handleCheckoutCompleted(supabase: ReturnType<typeof createClient>, session: Stripe.Checkout.Session) {
  ...
  const { data: existing, error: lookupError } = await findMemberProfile(supabase, {
    memberProfileId,
    email,
    stripeCustomerId,
  ...
  if (!existing) {
    console.warn("No member profile found for completed checkout", { memberProfileId, email, stripeCustomerId, stripeSubscriptionId });
    return;
  }
...
async function handleInvoicePaymentSucceeded(supabase: ReturnType<typeof createClient>, invoice: Stripe.Invoice) {
  ...
  if (lookupError) throw lookupError;
  if (!existing) {
    console.warn("No member profile found for paid invoice", { stripeCustomerId, stripeSubscriptionId });
    return;
  }
...
async function handleSubscriptionUpdated(supabase: ReturnType<typeof createClient>, subscription: Stripe.Subscription) {
  ...
  const { data: existing, error: lookupError } = await findMemberProfile(supabase, {
    memberProfileId: subscription.metadata?.member_profile_id,
  ...
  const paymentStatus = subscription.status === "active" || subscription.status === "trialing" ? "paid" : subscription.status;
  const accountStatus = subscription.status === "active" || subscription.status === "trialing"
    ? existing.profile_complete ? "active" : "pending_profile"
...
async function findMemberProfile(supabase: ReturnType<typeof createClient>, input: FindMemberInput) {
  ...
  if (input.memberProfileId) {
    return supabase.from("member_profiles").select(columns).eq("id", input.memberProfileId).maybeSingle();
  }

  if (input.stripeSubscriptionId) {
    return supabase.from("member_profiles").select(columns).eq("stripe_subscription_id", input.stripeSubscriptionId).maybeSingle();
  }

  if (input.stripeCustomerId) {
    return supabase.from("member_profiles").select(columns).eq("stripe_customer_id", input.stripeCustomerId).maybeSingle();
  }

  if (input.email) {
    return supabase.from("member_profiles").select(columns).eq("email", input.email).maybeSingle();
  }
