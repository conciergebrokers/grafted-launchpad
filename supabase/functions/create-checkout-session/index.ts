...
serve(async (req) => {
  ...
  try {
    ...
    if (!authorization) {
      return jsonResponse({ error: "Please sign in before continuing to payment." }, 401);
    }
    ...
    if (!email) {
      return jsonResponse({ error: "A member email is required before checkout." }, 400);
    }

    if (!body.public_directory_consent) {
      return jsonResponse({ error: "Public directory consent is required before checkout." }, 400);
    }
